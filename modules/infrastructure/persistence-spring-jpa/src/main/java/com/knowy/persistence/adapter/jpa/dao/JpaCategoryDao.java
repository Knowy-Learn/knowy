...
public interface JpaCategoryDao extends JpaRepository<CategoryEntity, Integer> {
	...
	Optional<CategoryEntity> findByName(@Param("name") String name) throws KnowyInconsistentDataException;

	@Query(value = "SELECT id, name FROM language l WHERE l.name ILIKE ANY (ARRAY[:names])", nativeQuery = true)
	Set<CategoryEntity> findByNameInIgnoreCase(@Param("names") String[] names) throws KnowyInconsistentDataException;
}
